...
<head>
  <meta charset="UTF-8" />
  <!-- Google Sign-In Client ID (레포지토리명 등 불필요 정보 제거) -->
  <meta name="google-signin-client_id" content="YOUR_GOOGLE_CLIENT_ID.apps.googleusercontent.com">
  <meta name="viewport" content="width=device-width, initial-scale=1.0"/>
  <title>3D 캐릭터 HUD, 달력, 구글 이메일 & 말풍선 채팅</title>
  ...
  <style>
    ...
    * {
    ...
    /* 오른쪽 HUD: 채팅 + 구글 이메일 전송 UI */
    #right-hud {
      position: fixed;
      top: 150px; /* 원하는 위치에 맞춰 조정 가능 */
      right: 10px;
      width: 300px;
      padding: 10px;
  ...
  <script>
    /* ====================================
       Google 로그인 & Gmail API
    ==================================== */
    // 전역 변수: 로그인 후 가져올 사용자 이메일
    ...
    function onSignIn(googleUser) {
      ...
      console.log("Google 로그인 성공, 이메일:", userProfileEmail);
      // 로그인 성공 후 Gmail API 초기화 로드
    ...
    function initGmailClient() {
      gapi.client.init({
        apiKey: "YOUR_API_KEY",  // 본인 API 키로 교체
    ...
    function sendGoogleEmail() {
      ...
      if (!userProfileEmail) {
        alert("먼저 구글 로그인 후 이메일 주소를 가져와주세요.");
        return;
      }
      const raw = createEmail(recipient, userProfileEmail, subject, message);
